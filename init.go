...
func init() {
	...
	config.K3SServerString = util.Getenv("K3S_SERVER_STRING", "--docker --cluster-cidr \"10.1.0.0/16\" --service-cidr \"10.2.0.0/16\" --cluster-dns \"10.2.0.10\" --disable=traefik,servicelb,metrics-server --cluster-init --snapshotter=native --flannel-backend=host-gw --flannel-iface=ethwe --kubelet-arg cgroup-driver=systemd ")
