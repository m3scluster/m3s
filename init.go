...
func init() {
	...
	config.DSDISKLimit, _ = strconv.ParseFloat(util.Getenv("DS_DISK_LIMIT", util.Getenv("DS_DISK", "10000")), 64)
	...
	config.K3SServerDISKLimit, _ = strconv.ParseFloat(util.Getenv("K3S_SERVER_DISK_LIMIT", util.Getenv("K3S_SERVER_DISK", "1000")), 64)
	config.K3SServerString = util.Getenv("K3S_SERVER_STRING", "/usr/local/bin/k3s server --cluster-cidr=10.2.0.0/16 --service-cidr=10.3.0.0/16 --cluster-dns=10.3.0.10 --kube-scheduler-arg=leader-elect=false --kube-controller-manager-arg=enable-leader-migration=false --kube-cloud-controller-manager-arg=enable-leader-migration=false --disable-cloud-controller --snapshotter=native --flannel-backend=vxlan")
	...
	config.K3SAgentString = util.Getenv("K3S_AGENT_STRING", "/usr/local/bin/k3s agent --snapshotter=native")
	...
	config.K3SAgentDISKLimit, _ = strconv.ParseFloat(util.Getenv("K3S_AGENT_DISK_LIMIT", util.Getenv("K3S_AGENT_DISK", "10000")), 64)
