...
func init() {
	...
	config.K3SServerString = util.Getenv("K3S_SERVER_STRING", "/usr/local/bin/k3s server --cluster-cidr=10.2.0.0/16 --service-cidr=10.3.0.0/16 --cluster-dns=10.3.0.10 --kube-scheduler-arg='leader-elect=false' --kube-controller-manager-arg='leader-elect=false' --disable-cloud-controller --snapshotter=native --flannel-backend=vxlan ")
